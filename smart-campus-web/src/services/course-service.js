...
(function (global) {
  ...
  const colorPalette = ['blue', 'green', 'orange', 'purple', 'pink'];
  ...
  const CourseService = {
    // 课程
    getCourses() { return load('courses', courseSample); },
    saveCourses(list) { save('courses', list); },
    addCourse(c) {
      const list = this.getCourses();
      if (list.some(x => x.id === c.id)) { Common.showMsg('课程编号已存在', 'error'); return false; }
      list.unshift(c);
      this.saveCourses(list);
      Common.showMsg('添加课程成功：' + c.name);
      return true;
    },
    updateCourse(c) {
      const list = this.getCourses();
      ...
      list[idx] = Object.assign({}, list[idx], c);
      this.saveCourses(list);
      Common.showMsg('课程信息更新成功');
      return true;
    },
    deleteCourse(id) {
      const list = this.getCourses().filter(x => x.id !== id);
      this.saveCourses(list);
      // 清理该课程相关排课
      const sched = this.getSchedule().filter(s => s.courseId !== id);
      this.saveSchedule(sched);
      Common.showMsg('删除课程成功（已清理相关排课）');
    },

    // 教室
    getRooms() { return load('rooms', roomSample); },
    saveRooms(list) { save('rooms', list); },
    addRoom(r) {
      const list = this.getRooms();
      if (list.some(x => x.id === r.id)) { Common.showMsg('教室编号已存在', 'error'); return false; }
      list.unshift(r);
      this.saveRooms(list);
      Common.showMsg('添加教室成功');
      return true;
    },
    updateRoom(r) {
      const list = this.getRooms();
      ...
      this.saveRooms(list);
      Common.showMsg('教室信息更新成功');
      return true;
    },
    deleteRoom(id) {
      const list = this.getRooms().filter(x => x.id !== id);
      this.saveRooms(list);
      Common.showMsg('删除教室成功');
    },

    // 实验室
    getLabs() { return load('labs', labSample); },
    saveLabs(list) { save('labs', list); },
    addLab(l) {
      const list = this.getLabs();
      if (list.some(x => x.id === l.id)) { Common.showMsg('实验室编号已存在', 'error'); return false; }
      list.unshift(l);
      this.saveLabs(list);
      Common.showMsg('添加实验室成功');
      return true;
    },
    updateLab(l) {
      const list = this.getLabs();
      ...
      list[idx] = Object.assign({}, list[idx], l);
      this.saveLabs(list);
      Common.showMsg('实验室信息更新成功');
      return true;
    },
    deleteLab(id) {
      const list = this.getLabs().filter(x => x.id !== id);
      this.saveLabs(list);
      Common.showMsg('删除实验室成功');
    },

    // 教学任务（排课）
    getSchedule() { return load('schedule', scheduleSample); },
    saveSchedule(list) { save('schedule', list); },
    addScheduleItem(item) {
      const list = this.getSchedule();
      const roomConflict = list.some(s => s.day === item.day && s.period === item.period && s.roomId === item.roomId);
      const teacherConflict = list.some(s => s.day === item.day && s.period === item.period && s.teacherId === item.teacherId);
      if (roomConflict) { Common.showMsg('教室在该时段已被占用，排课冲突！', 'error'); return false; }
      if (teacherConflict) { Common.showMsg('该教师在该时段已有课程，排课冲突！', 'error'); return false; }
      item.id = Common.uid('SCH');
      list.push(item);
      this.saveSchedule(list);
      Common.showMsg('排课成功：' + item.courseName);
      return true;
    },
    updateScheduleItem(item) {
      ...
      // 排除自身后的冲突检测
      const temp = list.filter(s => s.id !== item.id);
      const roomConflict = temp.some(s => s.day === item.day && s.period === item.period && s.roomId === item.roomId);
      const teacherConflict = temp.some(s => s.day === item.day && s.period === item.period && s.teacherId === item.teacherId);
      if (roomConflict) { Common.showMsg('教室时段冲突，无法更新', 'error'); return false; }
      if (teacherConflict) { Common.showMsg('教师时段冲突，无法更新', 'error'); return false; }
      list[idx] = Object.assign({}, list[idx], item);
      this.saveSchedule(list);
      Common.showMsg('教学任务更新成功');
      return true;
    },
    deleteScheduleItem(id) {
      const list = this.getSchedule().filter(s => s.id !== id);
      this.saveSchedule(list);
      Common.showMsg('已取消该教学任务');
    },
    autoArrange() {
      const courses = this.getCourses();
      const rooms = [...this.getRooms(), ...this.getLabs()];
      if (!courses.length || !rooms.length) {
        Common.showMsg('请先添加课程和教室/实验室', 'warning');
        return 0;
      }
      let existing = this.getSchedule();
      let created = 0;
      const colorPalette = ['blue', 'green', 'orange', 'purple', 'pink'];
      courses.forEach((course, idx) => {
        for (let times = 0; times < 2; times++) {
          ...
          for (let day = 1; day <= 5 && !placed; day++) {
            for (let period = 1; period <= 4 && !placed; period++) {
              if (existing.some(s => s.courseId === course.id && s.day === day && s.period === period)) continue;
              const room = rooms[(idx + day + period) % rooms.length];
              const roomConflict = existing.some(s => s.roomId === room.id && s.day === day && s.period === period);
              const teacherConflict = existing.some(s => s.teacherId === course.teacherId && s.day === day && s.period === period);
              if (roomConflict || teacherConflict) continue;
              existing.push({
                id: Common.uid('SCH'),
      ...
      Common.showMsg('智能排课完成，共生成 ' + created + ' 条排课记录', 'success');
      return created;
    },
    clearSchedule() {
      this.saveSchedule([]);
      Common.showMsg('课表已全部清空', 'warning');
    },
    ...
    // 合并所有可用房间（教室+实验室），用于排课时下拉选择
    getAllPlaces() {
      return [...this.getRooms(), ...this.getLabs()].map(p => ({ id: p.id, name: p.name, type: p.type }));
    },

    courseCount() { return this.getCourses().length; }
  };
